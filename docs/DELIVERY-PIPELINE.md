...
| `meta/routines/auto-merge-gate.prompt.md` | Repeatable prompt for guarded review, repair, and merge decisions. |
...
| Label | Meaning |
|---|---|
| `autonomous` | Issue has enough context, acceptance criteria, and test expectations for an agent to attempt. |
...
Do not apply `autonomous` until the issue is specific enough for a worker to
...
1. Select one open issue labeled `autonomous` and not labeled `in-progress`,
   `needs-human`, or `blocked`.
2. Re-read the issue, `docs/ROADMAP.md`, this pipeline doc, and relevant code.
3. Create a branch named `claude/issue-<number>-<slug>` for Claude routines, or
   `codex/issue-<number>-<slug>` for local Codex work.
...
For the first 10 autonomous Rafiki PRs, keep a human spot check before merge
and record the result in `meta/audits/dev-loop-log.csv`. After that, auto-merge
can be enabled only if the prior PRs show clean scope control, passing CI, and
useful self-checks.
...
Agents must check for pause signals before starting new work and before merge.
